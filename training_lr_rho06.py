...
if __name__ == "__main__":
    ...
    rng.seed(0)
    X = np.load("./data/X_rho06_N50_p50.npy")
    ...
    model = MLP_variant(N, p, [512, 512], 'leakyrelu').to(device)
    ...
    np.save(f'./results/linear regression/X_p{p}_q{int(1000*q)}_exp{exp_id}', X)
    np.save(f'./results/linear regression/mean_p{p}_q{int(1000*q)}_exp{exp_id}', mean)
    np.save(f'./results/linear regression/std_p{p}_q{int(1000*q)}_exp{exp_id}', std)
    np.save(f'./results/linear regression/train_losses_p{p}_q{int(1000*q)}_exp{exp_id}', train_losses)
    np.save(f'./results/linear regression/val_losses_p{p}_q{int(1000*q)}_exp{exp_id}', val_losses)
    np.save(f'./results/linear regression/coordinate_loss_p{p}_q{int(1000*q)}_exp{exp_id}', coordinate_loss)
    torch.save(model.state_dict(), f'./model/linear regression/p{p}_q{int(1000*q)}_exp{exp_id}.pt')
