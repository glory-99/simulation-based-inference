...
from simulators import Generator_logistic
import time
import argparse
...
if __name__ == "__main__":
    ...
    scheduler = torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[scheduler1, scheduler2], milestones=[950])
    train_losses, val_losses = train_model_with_generator(model, generator, optimizer, epochs=epochs,
                                        ...
                                        q=q, val_data=valid_dataloader, scheduler=scheduler,
                                        coordinate_loss=coordinate_loss, Y_test=Y_val, beta_test=beta_val)
    end_time = time.time()
    print("Trainging time: {:.2f}".format(end_time-start_time))
    np.save(f'./results/train_losses_p{p}_q{int(1000*q)}_log_exp{exp_id}', train_losses)
    np.save(f'./results/val_losses_p{p}_q{int(1000*q)}_log_exp{exp_id}', val_losses)
    np.save(f'./results/coordinate_loss_p{p}_q{int(1000*q)}_log_exp{exp_id}', coordinate_loss)
    torch.save(model.state_dict(), f'./model/p{p}_q{int(1000*q)}_log_exp{exp_id}.pt')
